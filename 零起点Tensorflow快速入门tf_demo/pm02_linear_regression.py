...
#4.1
print('\n#4.1,W变量权重、b偏离值，都使用tf.Variable变量格式')
print('使用线性回归模型:Y_pred = tf.add(tf.multiply(X, W), b)')
W = tf.Variable(tf.random_normal([1]), name='weight')
b = tf.Variable(tf.random_normal([1]), name='bias')
Y_pred = tf.add(tf.multiply(X, W), b)

#4.2
print('\n#4.2,Loss损失函数，也称cost代价函数')
print('用于衡量预测数值和实际数据之间的差距')
cost = tf.reduce_sum(tf.pow(Y_pred - Y, 2)) / (n_observations - 1)

#5
print('\n#5,设置learning_rate学习速率，用于optimizer优化函数')
...
#6
print('\n#6,创建graph图计算的session变量')
print('设置迭代次数，n_epochs = 1000')
n_epochs = 1000
with tf.Session() as sess:
    #6.1
    print('\n#6.1,初始化所有graph图计算的所有变量')
    ...
    for epoch_i in range(n_epochs):
        ...
        if dk < 0.000001:
            print('\n#6.6,模型误差值间隔小于0.000001，可以提前退出训练过程')
            print('\n@{0}#,dk,{1:.8f}'.format(epoch_i,dk))
            break
        
        #6.7
        #print('\n#6.7,保存训练误差到变量prev_training_cost')
